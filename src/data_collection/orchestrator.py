...
from src.data_collection.refresh_strategy import (
...
class DataCollectionOrchestrator:
    ...
    async def refresh_movie_data(
        ...
        for _, movie in movies_df.iterrows():
            ...
            last_tmdb = pd.to_datetime(movie['last_tmdb_update']) if pd.notna(movie['last_tmdb_update']) else None
            if last_tmdb and last_tmdb.tzinfo is None:
                last_tmdb = last_tmdb.replace(tzinfo=timezone.utc)
            
            last_omdb = pd.to_datetime(movie['last_omdb_update']) if pd.notna(movie['last_omdb_update']) else None
            if last_omdb and last_omdb.tzinfo is None:
                last_omdb = last_omdb.replace(tzinfo=timezone.utc)
            
            if should_freeze_movie(release_date, last_tmdb, last_omdb, consecutive_unchanged_cycles=3):
                self.db.execute(
